...
- data_metadata_v0.4.json: 元数据信息
- company_mapping_v0.4.json: 公司映射表
- processing_log_v0.4.txt: 处理日志
"""
...
# 版本信息
VERSION_SUFFIX = "v0.4"
METADATA_VERSION = "v0.4_20251220"
...
def collect_all_columns(excel_files, sample_size=10):
    ...
    # 移除要删除的列（包括COLS_TO_DROP和COLS_TO_EXTRACT）
    all_columns = all_columns - set(COLS_TO_DROP) - set(COLS_TO_EXTRACT)
    
    # 添加公司标识列
    all_columns.add('sequence_id')
    all_columns.add('company_name')
    all_columns.add('stock_code')
    all_columns.add('货币单位')
...
def process_all_files(excel_dir, output_dir, index_data_dict):
    ...
    metadata = {
        ...
        'columns': {
            ...
            'date_columns': ['日期'],
            'company_columns': ['sequence_id', 'company_name', 'stock_code', '货币单位']
        },
        'statistics': {
