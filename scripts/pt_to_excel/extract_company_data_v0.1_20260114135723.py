...
使用方法:
    ...
    3. 运行: python extract_company_data_v0.1_20260114135723.py
"""
...
def load_data_and_index(data_dir: Path):
    ...
    Returns:
        train_data, val_data, train_index_df, val_index_df
    """
    print("=" * 80)
    print("加载数据")
    print("=" * 80)
    ...
    return train_data, val_data, train_index_df, val_index_df
...
def extract_company_data(company_name, train_data, val_data, train_index_df, val_index_df):
    """
    提取指定公司的所有样本
...
def save_samples_to_excel(company_name, train_samples, val_samples, output_dir, max_sheets=500):
    ...
    for file_idx in range(num_files):
        if num_files > 1:
            filename = f"{safe_company_name}_extracted_data_v0.1_{timestamp}_part{file_idx+1}.xlsx"
        else:
            filename = f"{safe_company_name}_extracted_data_v0.1_{timestamp}.xlsx"
...
def main():
    """主函数"""
    print("\n" + "=" * 80)
    print("公司数据提取脚本 v0.1_20260114135723")
    print("=" * 80)
    ...
    # 加载数据
    train_data, val_data, train_index_df, val_index_df = load_data_and_index(data_dir)
    ...
    for company_name in matched_companies:
        ...
        train_samples, val_samples = extract_company_data(
            company_name, train_data, val_data, train_index_df, val_index_df
        )
