...
功能:
1. 读取预处理的.pt文件（train_v0.5.pt和val_v0.5.pt）
2. 根据指定范围提取样本数据（如10000-10010表示第10000到10010个样本）
3. 将3D tensor数据转换为2D表格（展平时间步维度）
4. 输出为Excel文件（包含训练集和验证集两个sheet）

使用方法:
    python pt_to_excel_v0.1_20251224130538.py \\
        --data_dir /path/to/preprocess_data_v0.5_20251224120208 \\
...
DATA_DIR = "/data/project_20251211/data/processed/preprocess_data_v0.5_20251225173341"

# 训练集样本范围（可选，格式: "start-end"，如 "10000-10010" 表示提取第10000到10010个样本）
# 设置为 None 表示不提取训练集
TRAIN_SAMPLES = "45000-46000"  # 示例：提取前11个样本（索引0到10）

# 验证集样本范围（可选，格式: "start-end"，如 "5000-5010" 表示提取第5000到5010个样本）
# 设置为 None 表示不提取验证集
VAL_SAMPLES = "10020-11000"  # 示例：提取前11个样本（索引0到10）
...
# ============================================================================
SCRIPT_VERSION = 'v0.1'
SCRIPT_TIMESTAMP = '20251224130538'
...
def extract_data(data: dict, start: int, end: int, data_type: str) -> tuple:
    ...
    Returns:
        (X_extracted, y_extracted, feature_columns) 元组
    """
    X = data['X']
    y = data['y']
    metadata = data['metadata']
    ...
    return X_extracted, y_extracted, feature_columns


def tensor_to_dataframe(X: torch.Tensor, y: torch.Tensor, feature_columns: list) -> pd.DataFrame:
    """
    ...
    # 转换X为numpy数组
    # 形状: [num_samples, seq_len, num_features] -> [num_samples * seq_len, num_features]
    print("\n重塑数据...")
    X_numpy = X.numpy()
    
    # 重塑为2D：将每个样本的时间步展开为行
    X_reshaped = X_numpy.reshape(num_samples * seq_len, num_features)
    ...
    for feat_name in feature_columns:
        # 只保留前30个字符
        feat_name_short = feat_name[:30] if len(feat_name) > 30 else feat_name
        simplified_columns.append(feat_name_short)
    
    df = pd.DataFrame(X_reshaped, columns=simplified_columns)
    
    # 添加sample_id列（标识每个样本）
    sample_ids = []
    for i in range(num_samples):
        sample_ids.extend([i] * seq_len)
    df.insert(0, 'sample_id', sample_ids)
    
    # 添加time_step列（标识时间步）
    time_steps = list(range(1, seq_len + 1)) * num_samples
    df.insert(1, 'time_step', time_steps)
    
    # 添加target列：只在每个样本的第一行填充，其他行为NaN
    print("\n添加target列...")
    target_values = []
    y_numpy = y.numpy().squeeze()
    
    for i in tqdm(range(num_samples), desc="处理target列", disable=num_samples < 100):
        # 第一行填充target值
        target_values.append(float(y_numpy[i]))
        # 其他99行填充NaN
        target_values.extend([np.nan] * (seq_len - 1))
    
    df['target'] = target_values
    
    print(f"\n  DataFrame形状: {df.shape}")
    print(f"  总行数: {df.shape[0]} (样本数{num_samples} × 序列长度{seq_len})")
    print(f"  总列数: {df.shape[1]} (sample_id + time_step + {num_features}个特征 + target)")
...
def generate_output_filename(train_range: str = None, val_range: str = None) -> str:
    """
    生成输出文件名
...
输出:
    ...
    if args.train_samples:
        ...
        try:
            ...
            # 提取数据
            X_train, y_train, feature_columns = extract_data(
                train_data, train_start, train_end, 'train'
            )
            
            # 转换为DataFrame
            train_df = tensor_to_dataframe(X_train, y_train, feature_columns)
    ...
    if args.val_samples:
        ...
        try:
            ...
            # 提取数据
            X_val, y_val, feature_columns = extract_data(
                val_data, val_start, val_end, 'val'
            )
            
            # 转换为DataFrame
            val_df = tensor_to_dataframe(X_val, y_val, feature_columns)
