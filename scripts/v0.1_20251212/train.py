...
training_path = project_root / "src" / "training" / "v0.1_20251212"
...
def get_latest_code_modify_time(config_version: str) -> str:
    ...
    # 2. 模型文件
    models_path = project_root / "src" / "models" / config_version
    files_to_check.append(models_path / "itransformer_decoder.py")
    
    # 3. 数据集文件
    data_path = project_root / "src" / "data" / config_version
    ...
    # 5. 工具文件
    utils_path = project_root / "src" / "utils" / config_version
    ...
    config_files = [
        config_dir / "dataset_config.yaml",
        config_dir / "itransformer_config.yaml",
...
def extract_version_number(config_version: str) -> str:
    ...
    Args:
        config_version: 配置版本号，如 'v0.1_20251212'
        
    Returns:
        版本号，如 'v0.1'
    """
    # 使用正则表达式提取版本号部分（v数字.数字）
    match = re.match(r'(v\d+\.\d+)', config_version)
    if match:
...
def get_data_generation_time(preprocessed_dir: Path = None, dataset_config: dict = None) -> str:
    ...
    if preprocessed_dir is not None:
        ...
        summary_file = preprocessed_dir / "preprocess_summary_v0.1_20251212.json"
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='训练iTransformer模型')
    parser.add_argument('--config_version', type=str, default='v0.1_20251212',
                        help='配置版本号')
    ...
    model_config_path = args.model_config or str(config_dir / "itransformer_config.yaml")
    training_config_path = args.training_config or str(config_dir / "training_config.yaml")
    dataset_config_path = args.dataset_config or str(config_dir / "dataset_config.yaml")
    
    print("=" * 80)
    print("iTransformer 训练脚本")
    print("=" * 80)
    ...
    if args.use_preprocessed:
        ...
        train_dataset = PreprocessedStockDataset(
            ...
            load_to_memory=True,
            device=None  # 先加载到CPU，训练时自动移到GPU
        )
    ...
    output_dir = project_root / "experiments" / f"itransformer_{version_number}_{code_timestamp}_{data_timestamp}"
    ...
    # 更新模型配置中的特征数量（如果不同）
    if model_cfg['input_features'] != num_features:
        print(f"\n警告: 配置中的特征数量 ({model_cfg['input_features']}) 与实际特征数量 ({num_features}) 不同")
        print(f"更新模型配置中的特征数量为: {num_features}")
        model_cfg['input_features'] = num_features
        
        # 自动调整输入ResNet第一个模块的维度（如果存在）
        if model_cfg.get('input_resnet', {}).get('enabled', False):
            modules = model_cfg['input_resnet'].get('modules', [])
            if modules and len(modules) > 0:
                first_module = modules[0]
                first_hidden_dims = first_module.get('hidden_dims', [])
                if first_hidden_dims and len(first_hidden_dims) > 0:
                    # 检查第一个模块的第一层输入维度是否匹配
                    if first_hidden_dims[0] != num_features:
                        print(f"\n自动调整输入ResNet第一个模块的维度:")
                        print(f"  配置中的维度: {first_hidden_dims[0]}")
                        print(f"  实际特征数量: {num_features}")
                        new_dims = [num_features] * len(first_hidden_dims)
                        print(f"  更新第一个模块: {first_hidden_dims} -> {new_dims}")
                        # 保持层数不变，将所有层的维度都调整为实际特征数
                        # 这样第一个模块就是：num_features -> num_features (N层)
                        first_module['hidden_dims'] = [num_features] * len(first_hidden_dims)
                        first_module['layers'] = len(first_hidden_dims)
    
    if model_cfg['seq_len'] != seq_len:
        print(f"\n警告: 配置中的序列长度 ({model_cfg['seq_len']}) 与实际序列长度 ({seq_len}) 不同")
    ...
    train_loader = DataLoader(
        ...
        prefetch_factor=train_cfg.get('prefetch_factor', 2),  # 数据预取，提升加载速度
        persistent_workers=train_cfg.get('persistent_workers', True)  # 保持worker进程
    )
    ...
    model = iTransformerDecoder(
        input_features=model_cfg['input_features'],  # 使用自动检测的特征数量
        ...
        input_resnet_config=model_cfg.get('input_resnet', {}),
        output_resnet_config=model_cfg.get('output_resnet', {}),
        final_output_config=model_cfg.get('final_output', {})
    )
