...
# 导入所需模块（使用v0.2版本的模型和trainer，v0.1版本的数据和utils）
models_path = project_root / "src" / "models" / "v0.2_20251225"
data_path = project_root / "src" / "data" / "v0.1_20251212"
training_path = project_root / "src" / "training" / "v0.2_20251225"
...
def get_latest_code_modify_time(config_version: str) -> str:
    ...
    # 2. 模型文件（使用v0.2版本）
    models_path = project_root / "src" / "models" / "v0.2_20251225"
    files_to_check.append(models_path / "tsmixer.py")
    files_to_check.append(models_path / "tsmixer_blocks.py")
    ...
    # 尝试添加其他配置文件（如果存在）
    for config_name in ["dataset_config.yaml", "tsmixer_config.yaml"]:
...
def extract_version_number(config_version: str) -> str:
    ...
    Args:
        config_version: 配置版本号，如 'v0.2_20251225'
        
    Returns:
        版本号，如 'v0.2'
    """
    # 使用split分割，取第一部分（下划线前的版本号）
    # 例如: 'v0.2_20251225' -> 'v0.2'
    #      'v0.2_test_20251225' -> 'v0.2_test'
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='训练TSMixer模型（集成TensorBoard）')
    parser.add_argument('--config_version', type=str, default='v0.2_20251225',
                        help='配置版本号')
    ...
    elif (config_dir / "tsmixer_config.yaml").exists():
        model_config_path = str(config_dir / "tsmixer_config.yaml")
    else:
    ...
    print("=" * 80)
    print("TSMixer 训练脚本 (集成TensorBoard)")
    print("=" * 80)
    ...
    output_dir = project_root / "experiments" / f"tsmixer_{version_number}_{code_timestamp}_{data_timestamp}"
    ...
    if tb_enabled:
        ...
        # 使用完整的模型文件夹名称作为运行名称
        # 例如: tsmixer_v0.2_20260106203122_20260106104018_50060
