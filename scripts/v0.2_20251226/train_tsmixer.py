...
# -*- coding: utf-8 -*-
"""
主训练脚本（集成TensorBoard）- TSMixer版本（单分支架构）
版本: v0.2
日期: 20251226

整合所有模块，作为训练入口，支持TensorBoard可视化
使用TSMixer单分支模型进行时间序列预测（MLP时间和特征混合）
"""
...
# 导入所需模块（使用v0.2_20251226版本的模型和trainer，v0.1版本的数据和utils）
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='训练TSMixer模型（单分支架构，MLP时间和特征混合，集成TensorBoard）')
    parser.add_argument('--config_version', type=str, default='v0.2_20251226',
                        help='配置版本号')
    ...
    print("=" * 80)
    print("TSMixer 训练脚本 (单分支架构 + MLP时间和特征混合 + TensorBoard)")
    print("=" * 80)
    ...
    model = TSMixer(
        ...
        ff_dim=model_cfg.get('ff_dim', 2048),  # 保留以兼容，实际架构固定
