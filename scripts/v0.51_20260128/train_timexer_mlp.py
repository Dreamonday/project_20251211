...
# -*- coding: utf-8 -*-
"""
主训练脚本 - TimeXer-MLP v0.51（学习型Missing Embedding）
版本: v0.51
日期: 20260128

使用TimeXer-MLP双分支模型，集成TensorBoard，支持学习型Missing Embedding
"""
...
# 导入模块（使用v0.51模型，v0.2 trainer，v0.1数据和utils）
models_path = project_root / "src" / "models" / "v0.51_20260128"
...
def get_feature_split_indices(preset: str, model_config: dict, n_features: int) -> tuple:
    """
    根据预设方案从配置文件返回内生和外生索引
    
    Args:
        preset: 预设方案名称 ("default", "subset10", 或 "custom")
        model_config: 模型配置字典（包含预设配置）
        n_features: 总特征数量
    ...
    Raises:
        ValueError: 如果预设方案未知或配置缺失
    """
    if preset == "default":
        # 从配置文件读取 default 预设
        preset_config = model_config.get('preset_default', {})
        if not preset_config:
            raise ValueError("preset_default 配置未找到，请检查配置文件")
        endogenous_indices = preset_config.get('endogenous_indices', [])
        exogenous_indices = preset_config.get('exogenous_indices', [])
    elif preset == "subset10":
        # 从配置文件读取 subset10 预设
        preset_config = model_config.get('preset_subset10', {})
        if not preset_config:
            raise ValueError("preset_subset10 配置未找到，请检查配置文件")
        endogenous_indices = preset_config.get('endogenous_indices', [])
        exogenous_indices = preset_config.get('exogenous_indices', [])
    elif preset == "custom":
        # 使用自定义索引配置
        endogenous_indices = model_config.get('endogenous_indices', None)
        exogenous_indices = model_config.get('exogenous_indices', None)
        if endogenous_indices is None or exogenous_indices is None:
            raise ValueError("使用 custom 预设时，必须在配置文件中指定 endogenous_indices 和 exogenous_indices")
    else:
        raise ValueError(f"Unknown feature_split_preset: {preset}. Available: 'default', 'subset10', 'custom'")
    
    # 验证索引列表
    if not endogenous_indices or not exogenous_indices:
        raise ValueError(f"索引列表为空，请检查配置文件中的 {preset} 配置")
    
    print(f"\n应用特征分离预设方案: '{preset}'")
    if len(endogenous_indices) <= 10:
        print(f"  内生索引: {endogenous_indices} (共{len(endogenous_indices)}个)")
    else:
        print(f"  内生索引: {endogenous_indices[:5]}...{endogenous_indices[-5:]} (共{len(endogenous_indices)}个)")
    
    if len(exogenous_indices) <= 10:
        print(f"  外生索引: {exogenous_indices} (共{len(exogenous_indices)}个)")
    else:
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='训练TimeXer-MLP模型 v0.51（学习型Missing Embedding）')
    parser.add_argument('--config_version', type=str, default='v0.51_20260128',
                        help='配置版本号')
    ...
    print("=" * 80)
    print("TimeXer-MLP v0.51 训练脚本（学习型Missing Embedding）")
    print("=" * 80)
    ...
    if preset is not None and preset != "custom":
        # 根据预设方案从配置文件读取索引列表
        endogenous_indices, exogenous_indices = get_feature_split_indices(preset, model_cfg, num_features)
        # 更新配置
        model_cfg['endogenous_indices'] = endogenous_indices
        model_cfg['exogenous_indices'] = exogenous_indices
    else:
        # 读取配置中的索引列表（custom 或未指定 preset）
    ...
    print(f"学习型Missing Embedding: 启用 (v0.51新特性，自动处理-1000缺失值)")
