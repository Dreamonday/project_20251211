...
# -*- coding: utf-8 -*-
"""
主训练脚本（集成TensorBoard）- TimeXer-MLP版本（双分支架构，MLP融合）
版本: v0.5
日期: 20260107

整合所有模块，作为训练入口，支持TensorBoard可视化
使用TimeXer-MLP双分支模型进行时间序列预测（内生+宏观，MLP融合替代Attention）
"""
...
# ===== 默认训练数据路径（如需调整请修改此处） =====
DEFAULT_PREPROCESSED_DIR = project_root / "data" / "processed" / "preprocess_data_v0.5_20260112120624_500120_more_continueTV"
...
# 导入所需模块（使用v0.5_20260107版本的模型，v0.2_20251226版本的trainer，v0.1版本的数据和utils）
models_path = project_root / "src" / "models" / "v0.5_20260107"
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='训练TimeXer-MLP模型（双分支架构，MLP融合，集成TensorBoard）')
    parser.add_argument('--config_version', type=str, default='v0.5_20260107',
                        help='配置版本号')
    ...
    print("=" * 80)
    print("TimeXer-MLP 训练脚本 (双分支架构 + MLP融合 + TensorBoard)")
    print("=" * 80)
    ...
    if tb_enabled:
        ...
        # 使用完整的模型文件夹名称作为运行名称
        # 例如: timexer_mlp_v0.5_20260107142054_20260106104708_500120
