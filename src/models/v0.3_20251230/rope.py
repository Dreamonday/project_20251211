...
class RotaryPositionEmbedding(nn.Module):
    ...
    def forward(
        ...
        if is_continuous:
            # 连续位置，直接使用缓存
            cos = self.cos_cache[:, :, :seq_len, :].expand(batch_size, n_heads, seq_len, d_k)
            sin = self.sin_cache[:, :, :seq_len, :].expand(batch_size, n_heads, seq_len, d_k)
        else:
            ...
            # 扩展维度以匹配缓存形状
            pos_ids_for_gather = pos_ids_expanded.expand(batch_size, n_heads, seq_len, d_k)
            # 从缓存中提取
            cos = torch.gather(
                self.cos_cache.expand(batch_size, n_heads, self.max_seq_len, d_k),
                dim=2,
                index=pos_ids_for_gather
            )
            sin = torch.gather(
                self.sin_cache.expand(batch_size, n_heads, self.max_seq_len, d_k),
                dim=2,
                index=pos_ids_for_gather
            )
        ...
        # 将cos和sin也分成两部分，分别对应x1和x2
        cos1, cos2 = cos.chunk(2, dim=-1)  # 每个都是 (batch, n_heads, seq_len, d_k//2)
        sin1, sin2 = sin.chunk(2, dim=-1)  # 每个都是 (batch, n_heads, seq_len, d_k//2)
        
        # 应用旋转：旋转后的x = [x1*cos1 - x2*sin1, x1*sin1 + x2*cos1]
        # 注意：cos和sin的前半部分和后半部分相同（因为freqs被拼接了两次）
        rotated_x = torch.cat([
            x1 * cos1 - x2 * sin1,
            x1 * sin1 + x2 * cos1
        ], dim=-1)
