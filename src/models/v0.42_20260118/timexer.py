...
v0.42新增：完整的mask支持，排除空白数据（-1000）影响
关键改进：
1. 时间聚合使用masked mean
2. TSMixer块传递mask
3. 注意力机制使用mask（在blocks中实现）
4. 使用MaskedLayerNorm
"""
...
try:
    ...
    MaskedLayerNorm = blocks_module.MaskedLayerNorm
...
class TimeXer(nn.Module):
    ...
    3. 共享时间混合层（处理500时间步）
    4. 内生分支: 3个LightweightTSMixerBlock → masked时间聚合 → [batch, 256]
    5. 宏观分支: 2个LightweightTSMixerBlock → masked时间聚合 → [batch, 256]
    6. 交叉注意力融合层（3层）
    7. 输出投影: 多层残差降维 → [batch, prediction_len]
    
    v0.42新增：全面支持mask机制
    - 时间聚合使用masked mean（关键改进）
    - TSMixerBlock传递mask
    - 使用MaskedLayerNorm
    """
    ...
    def __init__(
        ...
        if self.use_layernorm_before_pooling:
            # v0.42: 使用MaskedLayerNorm
            self.endogenous_norm = MaskedLayerNorm(endogenous_features)
        ...
        if self.use_layernorm_before_pooling:
            # v0.42: 使用MaskedLayerNorm
            self.exogenous_norm = MaskedLayerNorm(exogenous_features)
    ...
    def forward(
        ...
        if self.use_indices:
            ...
            # 同步分离mask
            if mask is not None:
                endogenous_mask = mask[:, :, self.endogenous_indices]  # (batch, 500, endogenous_features)
                exogenous_mask = mask[:, :, self.exogenous_indices]    # (batch, 500, exogenous_features)
            else:
                endogenous_mask = None
                exogenous_mask = None
        else:
            # 使用数量方式分离特征（向后兼容）
            ...
            # 同步分离mask
            if mask is not None:
                endogenous_mask = mask[:, :, :self.endogenous_features]  # (batch, 500, 44)
                exogenous_mask = mask[:, :, self.endogenous_features:]    # (batch, 500, 20)
            else:
                endogenous_mask = None
                exogenous_mask = None
        ...
        # 注意：投影后不传递mask
        # 原因：线性投影(y=Wx)会混合所有输入特征，无法精确追踪哪些是异常值
        # 即使原始44维中有部分-1000，投影后的48维是所有特征的线性组合
        # 在这个阶段让模型自由学习特征混合，不强制屏蔽
        # 投影回44维后，会使用原始endogenous_mask进行LayerNorm和聚合
        endogenous_mask_proj = None
        
        for mixer_block in self.endogenous_mixer_blocks:
            endogenous_x = mixer_block(endogenous_x, mask=endogenous_mask_proj)  # (batch, 500, 48)
        ...
        if self.endogenous_norm is not None:
            endogenous_x = self.endogenous_norm(endogenous_x, mask=endogenous_mask)
        
        # ========== 时间聚合（使用masked mean） ==========
        # 关键改进：通过mask排除异常值（如-1000），只对有效数据求平均
        if endogenous_mask is not None:
            # endogenous_x: (batch, 500, 44)
            # endogenous_mask: (batch, 500, 44)
            
            # 注意：这里必须将异常值位置置0后再求和
            # 虽然前面保留了异常值让模型学习，但聚合时必须排除它们
            # 否则-1000的极端值会污染聚合结果
            mask_float = endogenous_mask.float()
            masked_sum = (endogenous_x * mask_float).sum(dim=1)  # (batch, 44)
            
            # 计算每个特征的有效数据数量
            valid_counts = mask_float.sum(dim=1).clamp(min=1)  # (batch, 44)，防止除0
            
            # 计算masked mean（只对有效数据求平均）
            endogenous_x = masked_sum / valid_counts  # (batch, 44)
        else:
            # 如果没有mask，使用原有逻辑
        ...
        # 同内生分支，投影后不传递mask
        exogenous_mask_proj = None
        
        for mixer_block in self.exogenous_mixer_blocks:
            exogenous_x = mixer_block(exogenous_x, mask=exogenous_mask_proj)  # (batch, 500, 48)
        ...
        if self.exogenous_norm is not None:
            exogenous_x = self.exogenous_norm(exogenous_x, mask=exogenous_mask)
        
        # ========== 时间聚合（使用masked mean） ==========
        if exogenous_mask is not None:
            # 通过mask排除异常值，只对有效数据求平均
            mask_float = exogenous_mask.float()
            masked_sum = (exogenous_x * mask_float).sum(dim=1)  # (batch, 20)
            valid_counts = mask_float.sum(dim=1).clamp(min=1)  # (batch, 20)
            exogenous_x = masked_sum / valid_counts  # (batch, 20)
        else:
            # 标准mean
        ...
        # ========== 交叉注意力融合层 ==========
        # 注：此时已经聚合到batch维度，不再需要时间维度的mask
