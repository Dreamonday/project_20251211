...
使用方法:
    ...
    然后运行: python inference_by_company_v0.1_20260112133059.py
"""
...
def group_by_company_and_save(results: list, output_dir: Path):
    ...
    for (company_id, company_name, stock_code), group_df in tqdm(company_groups, desc="保存公司数据"):
        ...
        # 将统计信息转换为与output_df相同列数的DataFrame，以便追加
        # 创建一个与output_df列数相同的空行，然后填充统计信息
        # 使用pd.NA替代None，保持数据类型一致性
        stats_row_for_df = {}
        for col in output_df.columns:
            stats_row_for_df[col] = pd.NA
        
        # 添加统计信息到DataFrame末尾
        stats_row_df = pd.DataFrame([stats_row_for_df])
        # 在最后一行添加统计标记
        stats_row_df['统计标记'] = '统计汇总'
        stats_row_df['company_id'] = company_id
        stats_row_df['company_name'] = company_name
        stats_row_df['stock_code'] = stock_code
        
        # 添加统计列
        for key, value in stats_row.items():
            if key not in ['company_id', 'company_name', 'stock_code']:
                stats_row_df[key] = value
        
        # 确保stats_row_df的列顺序与output_df完全一致，避免FutureWarning
        stats_row_df = stats_row_df.reindex(columns=output_df.columns, fill_value=pd.NA)
        
        # 显式设置stats_row_df的列数据类型与output_df一致，避免FutureWarning
        # 使用try-except处理NA值无法转换的情况
        for col in output_df.columns:
            if col in stats_row_df.columns:
                try:
                    # 对于包含NA的列，使用更安全的转换方式
                    stats_row_df[col] = stats_row_df[col].astype(output_df[col].dtype)
                except (TypeError, ValueError):
                    # 转换失败时保持原样，这通常发生在列全为NA时
                    pass
