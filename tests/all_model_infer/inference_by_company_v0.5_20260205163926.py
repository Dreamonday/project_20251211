...
9. 计算收益率：基于输入数据最后一个交易日收盘价和模型预测收盘价
10. 统计和排名收益率指标

v0.5更新：
- 添加收益率计算：提取输入数据最后一个交易日的收盘价，计算预测收益率
- 在每个公司的Excel中添加收益率列
- 计算每家公司训练集和验证集的平均收益率
- 在排名文件中添加训练集和验证集平均收益率排名（从低到高）

使用方法:
    修改代码最前面的配置区域，设置MODEL_DIR和PREPROCESSED_DATA_DIR
    然后运行: python inference_by_company_v0.5_20260205163926.py
"""
...
# ===== 模型和数据配置（修改此处） =====
MODEL_DIR = "/data/project_20251211/experiments/timexer_mlp_v0.51_20260206190414_20260119170929_500120"
