...
2. 支持TimeXer新版本：v0.41_20260116、v0.42_20260118、v0.43_20260119、v0.44_20260126、v0.45_20260207
3. 支持TimeXerMLP新版本：v0.5_20260107、v0.53_20260207
4. 支持TimeXer-Official v0.6（官方架构 + Patch-Level内生 + Variate-Level外生）
5. 加载训练好的模型和对应的数据
6. 对所有训练集和验证集样本进行推理
7. 按公司分组，每个公司生成独立的Excel和Parquet文件
8. 计算绝对相对误差和相对误差（有正负值）
9. 计算每个公司训练集、验证集的统计指标（平均值和标准差）
10. 生成公司排名文件（按不同误差指标排名）
11. 计算收益率：基于输入数据最后一个交易日收盘价和模型预测收盘价
12. 统计和排名收益率指标

v0.7更新：
- 添加对TimeXer v0.45_20260207的支持（Instance Normalization + 反归一化）
- 添加对TimeXerMLP v0.53_20260207的支持（Instance Normalization + 反归一化）
- 新版本模型自动在内部进行归一化和反归一化
- 输出预测值已在原始尺度，无需额外处理

使用方法:
    修改代码最前面的配置区域，设置MODEL_DIR和PREPROCESSED_DATA_DIR
    然后运行: python inference_by_company_v0.7_20260207230002.py
"""
...
# ===== 模型和数据配置（修改此处） =====
MODEL_DIR = "/data/project_20251211/experiments/timexer_v0.45_20260211180709_20260119170929_500120"
...
def load_model_dynamically(model_dir: Path, device: str = 'cuda'):
    ...
    if has_norm_mask and model_type == 'timexer_mlp':
        # 检测到norm_mask，使用v0.53版本（支持Instance Normalization + Missing Embedding）
        actual_model_version = 'v0.53_20260207'
        models_path = project_root / "src" / "models" / actual_model_version
        print(f"检测到norm_mask参数，使用模型版本: {actual_model_version}（Instance Normalization + Missing Embedding）")
    elif has_missing_embedding and model_type == 'timexer_mlp':
        # 检测到missing_embedding但没有norm_mask，使用v0.51版本（只支持Missing Embedding）
        actual_model_version = 'v0.51_20260128'
        models_path = project_root / "src" / "models" / actual_model_version
        print(f"检测到missing_embedding参数，使用模型版本: {actual_model_version}（Missing Embedding）")
        ...
        # v0.7新增：v0.53使用对应版本的模型代码（Instance Normalization）
        ...
        # v0.7新增：v0.45使用对应版本的模型代码（Instance Normalization）
    ...
    if model_type == 'itransformer':
        ...
        # v0.7新增：支持Instance Normalization参数（v0.53模型）
        model = ModelClass(
            seq_len=model_config['seq_len'],
            n_features=model_config['n_features'],
            ...
            # v0.7新增：Instance Normalization参数
        ...
        # v0.7新增：v0.45也使用细粒度LayerNorm控制
        if model_version in ['v0.41_20260116', 'v0.42_20260118', 'v0.43_20260119', 'v0.44_20260126', 'v0.45_20260207']:
        ...
        # v0.7新增：v0.45的Instance Normalization参数
        ...
        # v0.45的Instance Normalization在模型内部完成，输出已是原始尺度
...
def inference_all_samples(
    ...
    # v0.7注意：v0.45和v0.53模型使用Instance Normalization，模型内部自动反归一化，输出已在原始尺度
    ...
    # v0.7新增：提示Instance Normalization模型
    if model_version in ['v0.45_20260207', 'v0.53_20260207']:
        print(f"\n*** 检测到{model_version}模型（Instance Normalization） ***")
        print(f"*** 模型内部自动进行归一化和反归一化 ***")
        print(f"*** 输出预测值已在原始尺度，无需额外处理 ***")
    ...
    if train_data is not None and train_index_df is not None:
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(0, len(train_X), batch_size), desc="训练集推理"):
                ...
                # v0.7注意：v0.45和v0.53模型输出已在原始尺度，无需还原
