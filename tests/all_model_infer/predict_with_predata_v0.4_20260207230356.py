...
使用方法：
    ...
    然后运行: python predict_with_predata_v0.4_20260207230356.py
"""
...
# 预测数据文件（.pt文件路径）
PREDATA_FILE = "/data/project_20251211/data/pre_data/processed_data_20260118_v0.2_20250101/data.pt"

# 误差排名文件路径（可选，如果不需要可以设为None或空字符串）
ERROR_RANKING_FILE = "/data/project_20251211/tests/inference_results/timexer_v0.45_20260211180709_20260119170929_500120_20260211192807/不同误差下的公司排名.parquet"
...
def load_model_dynamically(model_dir: Path, device: str = 'cuda'):
    ...
    # 根据参数特征智能选择模型代码路径
    # v0.4修复：优先处理明确的版本号，避免v0.53/v0.45被错误降级到v0.51
    if model_version == 'v0.53_20260207':
        # v0.4新增：v0.53使用对应版本的模型代码（Instance Normalization）
        ...
        # v0.4新增：v0.45使用对应版本的模型代码（Instance Normalization）
        ...
        # 检测到missing_embedding，使用v0.51版本的模型定义（仅用于v0.51之前的旧版本）
    ...
    if model_type == 'itransformer':
        ...
        # v0.4新增：支持Instance Normalization参数（v0.53模型）
        model = ModelClass(
            seq_len=model_config['seq_len'],
            n_features=model_config['n_features'],
            ...
            # v0.4新增：Instance Normalization参数
        ...
        # v0.4新增：v0.45的Instance Normalization参数
...
def predict_and_merge(
    ...
    # v0.4注意：v0.45和v0.53模型使用Instance Normalization，模型内部自动反归一化，输出已在原始尺度
    ...
    # v0.4新增：提示Instance Normalization模型
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(X), batch_size), desc="预测进度"):
            ...
            # v0.4注意：v0.45和v0.53模型输出已在原始尺度，无需还原
    ...
    for idx, company_info in enumerate(company_info_list):
        ...
        # 预测收盘价（已经过对数还原处理，如果是v0.44模型；或已反归一化，如果是v0.45/v0.53模型）
